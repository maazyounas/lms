...
const StudentGrades = ({ student }: Props) => {
  ...
  const subjectData = subjects.map((subj) => {
    ...
    const avgPct = tests.reduce((a, t) => a + (t.marks / t.total) * 100, 0) / tests.length;
    const overallGrade = avgPct >= 90 ? "A+" : avgPct >= 85 ? "A" : avgPct >= 80 ? "B+" : avgPct >= 75 ? "B" : avgPct >= 70 ? "C+" : avgPct >= 60 ? "C" : "D";
    ...
    <div>
      ...
      <div className="space-y-4">
        {subjectData.map((sub) => {
          ...
          return (
            <div key={sub.subject} className="bg-card border border-border rounded-xl overflow-hidden">
              {/* Subject Header */}
              <button
              ...
              >
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <p className="font-semibold text-foreground">{sub.subject}</p>
                    <p className="text-xs text-muted-foreground">{sub.tests.length} tests/quizzes</p>
                  </div>
                </div>
                <div className="flex items-center gap-4">
                  <div className="text-right mr-2">
                    <p className="text-sm text-muted-foreground">Average</p>
                    <p className="text-lg font-bold text-foreground">{sub.avgPct.toFixed(0)}%</p>
                  </div>
                  <span className={`px-3 py-1 rounded-full text-sm font-bold ${gradeColor(sub.overallGrade)} ${gradeBg(sub.overallGrade)}`}>
                    {sub.overallGrade}
                  </span>
                      ...
                      <tbody>
                        {sub.tests.map((t, i) => (
                          <tr key={i} className="border-b border-border last:border-0 hover:bg-muted/30 transition-colors">
                            <td className="px-5 py-3 text-sm text-foreground">{t.test}</td>
                            <td className="px-5 py-3 text-sm text-muted-foreground">{t.date}</td>
                            <td className="px-5 py-3 text-sm text-foreground font-medium">{t.marks}/{t.total}</td>
                            <td className="px-5 py-3 text-sm text-muted-foreground">{((t.marks / t.total) * 100).toFixed(0)}%</td>
                            <td className="px-5 py-3">
                              <span className={`font-bold text-sm ${gradeColor(t.grade)}`}>{t.grade}</span>
                            </td>
                          </tr>
                        ))}
                      </tbody>
